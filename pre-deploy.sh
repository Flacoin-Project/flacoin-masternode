...
if [[ $HOST = "x86_64-apple-darwin11" ]]; then
    find $TRAVIS_BUILD_DIR -type f | grep -i flacoin-qt.dmg$ | xargs -i cp {} $OUTDIR/bin
    find $TRAVIS_BUILD_DIR -type f | grep -i flacoin-core.dmg$ | xargs -i cp {} $OUTDIR/bin
else
    cp $TRAVIS_BUILD_DIR/src/qt/flacoin-qt $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/qt/flacoin-qt.exe $OUTDIR/bin/ || echo "no QT Wallet"
    cp $TRAVIS_BUILD_DIR/src/flacoind $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/flacoin.exe $OUTDIR/bin/
    cp $TRAVIS_BUILD_DIR/src/flacoin-cli $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/flacoin-cli.exe $OUTDIR/bin/
