...
"""
burn-fla: create a bitcoin burn address
By James C. Stroud
...
Call the program with a template burn address as the only argument::
    % burn-fla LZBurnAddressForFlaPreminexxxxxxx
    LZBurnAddressForFlaPreminextStefG
For instructions, call the program with no arguments::
    % burn-fla
    usage: burn-fla TEMPLATE
...
class BurnflaError(Exception):
    pass


class AlphabetError(BurnflaError):
    pass
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        usage()
    if sys.argv[1] == "test":
        template = "RXBurnAddressForFlaPreminexxxxxxx"
    else:
        template = sys.argv[1]
    for c in template:
        if c not in ABET:
