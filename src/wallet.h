...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 FLACOIN at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zFLA send
enum ZerocoinSpendStatus {
    ZFLA_SPEND_OKAY = 0,                            // No error
    ZFLA_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZFLA_WALLET_LOCKED = 2,                         // Wallet was locked
    ZFLA_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZFLA_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZFLA_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZFLA_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZFLA_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZFLA_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZFLA_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZFLA_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZFLA_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZFLA_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZFLA_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZFLA_SPENT_USED_ZFLA = 14,                      // Coin has already been spend
    ZFLA_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZFLA_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZFlaBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZFLAOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzflaAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzFLATracker> zflaTracker;
    ...
    void setZWallet(CzFLAWallet* zwallet)
    {
        zwalletMain = zwallet;
        zflaTracker = std::unique_ptr<CzFLATracker>(new CzFLATracker(strWalletFile));
    }

    CzFLAWallet* getZWallet() { return zwalletMain; }
    ...
    void setZFlaAutoBackups(bool fEnabled)
    {
    ...
    /** zFLA reset */
    boost::signals2::signal<void()> NotifyzFLAReset;
