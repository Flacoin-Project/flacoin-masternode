...
CzFLATracker::CzFLATracker(std::string strWalletFile)
{
...
CzFLATracker::~CzFLATracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzFLATracker::Init()
{
...
bool CzFLATracker::Archive(CMintMeta& meta)
{
...
bool CzFLATracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzFLATracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzFLATracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzFLATracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzFLATracker::GetSerialHashes()
{
...
CAmount CzFLATracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzFLATracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzFLATracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzFLATracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzFLATracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzFLATracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzFLATracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzFLATracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzFLATracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzFLATracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzFLATracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzFLATracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzFLATracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzFLATracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzFLATracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzFLATracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzFLATracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzfla from DB\n", __func__, listDeterministicDB.size());
    }
