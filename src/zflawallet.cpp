...
CzFLAWallet::CzFLAWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zfla seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZFLASeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZFLASeed_deprecated()) {
                    LogPrintf("%s: Updated zFLA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zfla seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zfla wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzFLAWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZFLACount(nCountLastUsed);
    else if (!walletdb.ReadZFLACount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzFLAWallet::Lock()
{
    seedMaster = 0;
}

void CzFLAWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzFLAWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZFLA(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzFLAWallet::LoadMintPoolFromDB()
{
...
void CzFLAWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzFLAWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzFLAWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zflaTracker->HasPubcoinHash(pMint.first)) {
...
bool CzFLAWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZFLA(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zflaTracker which also adds to database
    pwalletMain->zflaTracker->Add(dMint, true);
...
void CzFLAWallet::SeedToZFLA(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzFLAWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzFLAWallet::GenerateDeterministicZFLA(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzFLAWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzFLAWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
