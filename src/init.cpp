...
#include "config/flacoin-config.h"
#endif
...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightZflaThread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "flacoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "flacoind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the FLACOIN and zFLA money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 46800, 10500));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in FLACOIN/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in FLACOIN/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, flacoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in FLACOIN/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all FLACOIN specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-flastake=<n>", strprintf(_("Enable or disable staking functionality for FLACOIN inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zflastake=<n>", strprintf(_("Enable or disable staking functionality for zFLA inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:46800"));
    ...
    strUsage += HelpMessageOpt("-backupzfla=<n>", strprintf(_("Enable automatic wallet backups triggered after each zFLA minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zflabackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zFLA backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeflacoinamount=<n>     " + strprintf(_("Keep N FLACOIN anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 45300, 11000));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2019-%i The FLA Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2020-%i The Flacoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that FLACOIN is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Flacoin Core is shutting down."));
...
#endif
    // Make sure only a single FLACOIN process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Flacoin Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    if (proxyArg != "" && proxyArg != "0") {
        CService proxyAddr;
        if (!Lookup(proxyArg.c_str(), proxyAddr, 46800, fNameLookup)) {
    ...
    if (onionArg != "") {
        if (onionArg == "0") { // Handle -noonion/-onion=0
            ...
            CService onionProxy;
            if (!Lookup(onionArg.c_str(), onionProxy, 46800, fNameLookup)) {
    ...
    //FLACOIN: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //FLACOIN specific: zerocoin and spork DB's
                ...
                // FLACOIN: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    //if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZFLAMinted();
                        RecalculateZFLASpent();
                    //}
                    RecalculateFLACOINSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    //if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        ...
                        // FLACOIN: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Flacoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Flacoin Core to complete") << "\n";
        ...
        zwalletMain = new CzFLAWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zFLAWallet
        uiInterface.InitMessage(_("Syncing zFLA wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZFlaBackups = GetBoolArg("-backupzfla", true);
        pwalletMain->setZFlaAutoBackups(fEnableZFlaBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zflaTracker->Init();
...
//    }
//
//    nAnonymizeFlacoinAmount = GetArg("-anonymizeflacoinamount", 0);
//    if (nAnonymizeFlacoinAmount > 999999) nAnonymizeFlacoinAmount = 999999;
//    if (nAnonymizeFlacoinAmount < 2) nAnonymizeFlacoinAmount = 2;
    ...
    LogPrintf("Anonymize FLACOIN Amount %d\n", nAnonymizeFlacoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1FLACOIN+1000 == (.1FLACOIN+100)*10
       10FLACOIN+10000 == (1FLACOIN+1000)*10
    */
    ...
    */
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightZflaThread(threadGroup);
    }
