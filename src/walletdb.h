...
public:
    ...
    bool WriteZFLASeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZFLASeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZFLASeed_deprecated(uint256& seed);
    bool EraseZFLASeed();
    bool EraseZFLASeed_deprecated();

    bool WriteZFLACount(const uint32_t& nCount);
    bool ReadZFLACount(uint32_t& nCount);
