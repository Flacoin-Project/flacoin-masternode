...
#ifndef FLACOIN_ZFLAWALLET_H
#define FLACOIN_ZFLAWALLET_H
...
public:
    CzFLAWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZFLA(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZFLA(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //FLACOIN_ZFLAWALLET_H
