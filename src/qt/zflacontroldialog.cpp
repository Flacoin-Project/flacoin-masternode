...
#include "ui_zflacontroldialog.h"
...
std::set<std::string> ZFlaControlDialog::setSelectedMints;
std::set<CMintMeta> ZFlaControlDialog::setMints;

bool CZFlaControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZFlaControlDialog::COLUMN_DENOMINATION || column == ZFlaControlDialog::COLUMN_VERSION || column == ZFlaControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZFlaControlDialog::ZFlaControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZFlaControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZFlaControlDialog::~ZFlaControlDialog()
{
    delete ui;
}

void ZFlaControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZFlaControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZFlaControlWidgetItem* itemDenom(new CZFlaControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZFlaControlWidgetItem *itemMint = new CZFlaControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZFlaControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZFlaControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZFla_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZFlaControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZFlaControlDialog::GetSelectedMints()
{
...
void ZFlaControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
