...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::FLA);
    ...
    if (!settings.contains("nAnonymizeFlacoinAmount"))
        settings.setValue("nAnonymizeFlacoinAmount", 1000);

    nAnonymizeFlacoinAmount = settings.value("nAnonymizeFlacoinAmount").toLongLong();
...
// Wallet
    ...
    if (!settings.contains("addrProxy"))
        settings.setValue("addrProxy", "127.0.0.1:17000");
    ...
    if (settings.contains("nAnonymizeFlacoinAmount"))
        SoftSetArg("-anonymizeflacoinamount", settings.value("nAnonymizeFlacoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in flacoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFlacoinAmount:
            return QVariant(nAnonymizeFlacoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeFlacoinAmount:
            nAnonymizeFlacoinAmount = value.toInt();
            settings.setValue("nAnonymizeFlacoinAmount", nAnonymizeFlacoinAmount);
            emit anonymizeFlacoinAmountChanged(nAnonymizeFlacoinAmount);
            break;
        case CoinControlFeatures:
