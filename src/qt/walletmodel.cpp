...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered flacoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 FLA.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal flacoin:URI (flacoin:XyZ...?message=example)
            {
...
static void NotifyzFLAReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzFLAReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzFLAReset.connect(boost::bind(NotifyzFLAReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzFLAReset.disconnect(boost::bind(NotifyzFLAReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zflaTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
